...
try:
    ...
    st.error(
        "The application could not load the data files."
    )
...
if not varieties:
    ...
    st.warning(
        "No varieties have been added for this crop."
    )
...
moisture = st.number_input(
    ...
    help=(
        "Enter the moisture reading from "
        "your grain moisture meter."
    )
)
...
days_since_harvest = st.number_input(
    "How many days ago was it harvested?",
    ...
    help=(
        "For example, enter 5 if the crop "
        "was harvested 5 days ago."
    )
)
...
if not storage_methods:
    ...
    st.warning(
        "No storage methods have been added "
        "for this crop."
    )
...
if st.button(
    ...
    if duration_result["duration"]:
        ...
        if duration_result[
            ...
            st.warning(
                "The exact number of days since "
                "harvest is not currently in the "
                "duration dataset."
            )


            st.write(
                "The system therefore used the "
                "closest available rule based on "
    ...
    st.caption(
        """
        This is an MVP rule-based agricultural
